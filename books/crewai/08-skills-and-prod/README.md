...
**模式 1：只 Skills**（只给方法论，不需要动作）——纯写场景，比如技术写作员：
...
**模式 2：只 Tools**（只给动作，不需要特殊方法论）——通用研究场景：
...
**模式 3：Skills + Tools**（最常见）——安全分析师，既有审计方法论又有调研能力：
...
`SKILL.md` body 超过 **50,000 字符**会有软警告。没有硬限制，但太大的 skill 注入会稀释 Agent 注意力。**经验**：单个 skill body 控制在 5,000-10,000 字符。超长内容放 `references/` 目录，让 Skill 主体引用它们。

**3 个常见坑**

**坑 1：`allowed-tools` 字段是 experimental**——这只是 metadata，不会自动给 Agent 装这些 tools。**必须自己配 `tools=[...]`**。

**坑 2：目录名跟 `name` 字段不一致**——启动时 CrewAI 找不到这个 skill。**改目录名或改 name 字段保持一致**。

**坑 3：Skill 太多**——一个 Agent 挂了 10 个 skills，每个注入 prompt 1-2KB，10 个就是 20KB context。**挑 2-3 个最相关的**。
...
启动后所有 `crew.kickoff()` 调用自动 trace。**推荐**：本地开发用 Arize Phoenix（Docker 一键起），生产用 Langfuse Cloud 或自托管。
...
**Model routing**——80% 的子任务用 gpt-4o-mini 跑，关键决策（reasoning / final review）才上 gpt-4o。
...
**反模式 1：本地跑通直接上生产**——本地没考虑限流（max_rpm）、超时（tool timeout）、错误重试、监控（observability）、成本上限（token_budget）。

**反模式 2：prompt 写在代码里**——`agent = Agent(role=..., goal=..., backstory=...)` 改个 prompt 要重新部署。**修复**：用 YAML 配置（ch01 推的写法）。

**反模式 3：所有 Agent 用同一个强模型**——3 个 Agent × gpt-4o = 3 倍成本。研究员和写作员 gpt-4o-mini 够用，reviewer 才上 gpt-4o。

**反模式 4：不用 Skills / Knowledge**——把领域知识硬塞进 prompt 字符串。**修复**：用 Skill 把规范放文件系统，prompt 简洁。

**反模式 5：没设 guardrail**——Agent 输出没校验就直接用。生产里要 Pydantic 锁字段 + Guardrail 业务校验（ch06）。
