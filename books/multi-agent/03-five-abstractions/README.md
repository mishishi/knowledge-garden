...
`role` 是一个**短标签**，`backstory` 是给 LLM 看的**详细背景**。两者共同决定 LLM 的"人格"。

### LangGraph

LangGraph 没有显式的 Role 概念，而是把 Role 隐含在 **节点函数** 里：
...
```python
# 反模式 1：太宽
goal="帮助用户"  # 啥都能算"帮助"，没法验证

# 反模式 2：太窄
goal="输出第 47 个字符是 '你' 的字符串"  # 太死板，没有意义

# 反模式 3：矛盾
goal="用 100 字以内的篇幅完整介绍 Multi-Agent 的 5 个核心抽象的所有细节"
# 100 字根本讲不完 5 个抽象 → 目标互相矛盾
```
...
工具的关键是 **docstring 要写清楚**——LLM 是靠 docstring 决定要不要调用、调什么参数的。
...
完整代码：[`code/03_tool.py`](./code/03_tool.py)
...
长期记忆（Long-term）
└─ 跨会话保留的信息。比如"这个用户上次问了什么"。

共享记忆（Shared）
└─ 多个 Agent 之间共享的状态。比如"Researcher 的输出给 Writer 用"。
```
...
agent = Agent(
    role="...",
    memory=True,  # ← 启用长期记忆（向量数据库）
    # 默认会读 conversation_buffer + entity_memory + short_term
)
```

### LangGraph 的 Memory 实现

LangGraph 用 `State` 对象传递记忆：
...
完整代码：[`code/04_memory.py`](./code/04_memory.py)
...
完整代码：[`code/05_handoff.py`](./code/05_handoff.py)
...
- 5 个核心抽象：**Role / Goal / Tool / Memory / Handoff**
- 所有 Multi-Agent 框架都是这 5 个抽象的实现
- 设计顺序：Role → Goal → Tool → Memory → Handoff
