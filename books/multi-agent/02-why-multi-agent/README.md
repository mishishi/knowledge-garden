...
> 第 1 篇我们用单 Agent 查了天气、用 Multi-Agent 写了短文。本章具体拆解：单 Agent 在哪些场景下会"撑不住"，以及 Multi-Agent 如何解。

## 这一章要回答的问题

1. 单 Agent 的 3 个真实局限是什么？
...
- 单 Agent 的 3 个局限：上下文爆炸 / 角色冲突 / 调试地狱
- Multi-Agent 的解法：拆分角色 → 隔离 context → 独立可调试
- 决策树：先单 Agent，撑不住再拆
- 复杂度代价：Token、延迟、状态、错误、调试、测试都会增加

## 下篇

[03. 5 个核心抽象](../03-five-abstractions/) —— 把"怎么拆 Agent"这件事抽象成 5 个核心概念，所有框架都是它们的实现。
...
这些会在 [第 8 章 可观测性与成本](../08-observability-and-cost/) 和 [第 10 章 生产化 Checklist](../10-production-checklist/) 展开。