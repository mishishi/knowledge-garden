...
**persona effect**：让模型「扮演」某个具体角色比「你是一个 helpful assistant」效果好。具体角色（资深工程师 / 10 年律师 / 翻译）激活对应领域行为模式。
...
**Context 决定输出的针对性**——同样 task，给「电商平台客服」和「品牌营销人员」是不同输出。
...
动词要可验证：分析 / 列出 / 总结 / 重写 / 翻译。避免「看看 / 想一下 / 优化」这种模糊词。
...
**Format 决定下游可解析性**。JSON / markdown / 列表 / 表格——选对格式能省下游 50% 处理代码。
...
- 1 个中文字 ≈ 1.5 token
- 1 行代码 ≈ 10-30 token
- 1 张图片（1024x1024）≈ 85-255 token（按 tile 计费）
...
```
3 段 prompt × 2000 input + 1 段 response × 500 output

GPT-4o-mini：
  (3 × 2000 / 1M) × 0.15 + (500 / 1M) × 0.60
  = 0.0009 + 0.0003
  = $0.0012 / 次
...
**100 万次调用 = GPT-4o-mini $1,200，GPT-4o $6,000，o3 $38,000。**
...
1. **重复 system prompt**：每条消息都带 system，10 轮对话 = 10 次 system token
2. **大 context 不缓存**：5000 token context 跑 100 次 = 500,000 token input
3. **reasoning model 滥用**：o3 处理「翻译这句话」就是烧钱

**修复**：

1. → 用 conversation history 复用（不是每条都重发 system）
2. → **Prompt caching**（ch07 细讲）
3. → 简单任务用 mini / haiku / flash，**reasoning 留给复杂任务**
...
- 长 context（200K）下表现好
- 写作质量高
- 不喜欢「step by step think」强制 CoT（reasoning 风格）
...
**Prompt 风格**：
- 多模态描述要详细（"图中的蓝色框"，不能只说"这个"）
- 适合做多模态分析任务

### Reasoning model（o3 / Claude with thinking）

**特点**：
- 自动「思考」，不强制要 CoT prompt
- 慢（10-60s）
- 贵
- 简单任务反而效果差（容易过度思考）
...
跟 LLM 不要「客气」——它不会因为你语气好就更努力。直接给任务。
...
不指定长度 → LLM 默认 500-2000 字，看心情。**长 prompt 慢、贵、容易啰嗦**。

## 跑不起来的常见坑

**坑 1：以为 prompt 是自然语言**

Prompt 是给 LLM 看的「配置文件」，不是给人看的「聊天」。**结构化 + 标题分块 + 明确格式**永远比散文式 prompt 好。

**坑 2：prompt 在多轮对话里越加越长**

```python
# 多轮对话累积 prompt
messages = [
    {"role": "system", "content": "你是...你是...你是...（500 字）"},
...
每条都带 500 字 system → 100 轮对话 = 50,000 token system。**system 写短**（100 字以内），长背景放 context 或 knowledge。

**坑 3：跨模型复制 prompt**

GPT-4o 写得好的 prompt 不一定在 Claude 上好。**不同模型对「指令」的解析风格不同**——拿到好 prompt 后要在目标模型上跑测试。
...
- 4 元素（ROLE / CONTEXT / TASK / FORMAT）的具体写法
- Token 怎么算钱，3 个常见烧钱点
- 不同模型 prompt 风格的差异
- 3 个常见错误 + 3 个常见坑
- 6 项 prompt 写完检查清单
...
[02. Few-shot 实战](../02-few-shot/) — 怎么选样 + 4 大反模式 + 真实 case 跑出来的数据。
