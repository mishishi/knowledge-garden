...
.progress {
    ...
    transform: scaleX(0);
    z-index: 200;
    box-shadow: 0 1px 6px rgba(176, 137, 104, 0.35);
    transition: transform 0.08s linear, opacity 0.2s;
}
body.dark .progress { box-shadow: 0 1px 6px rgba(196, 168, 124, 0.4); }
...
/* 暗色模式: 黄闪降低饱和度避免刺眼 + 文本高亮加深背景 */
body.dark mark.kb-jump-flash { background: rgba(196, 168, 124, 0.45); color: #f5e6c8; }
body.dark .kb-result-text mark { background: rgba(196, 168, 124, 0.3); color: #f5e6c8; }
body.dark .notes-list mark { background: rgba(196, 168, 124, 0.3); color: #f5e6c8; }
body.dark .command-snippet mark { background: rgba(196, 168, 124, 0.3); color: #f5e6c8; }
body.dark mark.search-highlight { background: rgba(196, 168, 124, 0.25); color: #f5e6c8; }
...
.sidebar-bookmarks .sb-empty-icon {
    font-size: 24px;
    margin-bottom: 6px;
    opacity: 0.5;
}
.sidebar-bookmarks .sb-empty-title {
    font-size: 12px;
    color: var(--text-soft);
    margin-bottom: 6px;
    font-weight: 500;
}
.sidebar-bookmarks .sb-empty-hint {
    font-size: 11px;
    line-height: 1.6;
}
.notes-empty {
...
/* 书架 — content-visibility 跳过屏幕外组渲染, 大幅提升 17 系列滚动 */
.bookshelf .book-group {
    margin-bottom: 24px;
    content-visibility: auto;
    contain-intrinsic-size: auto 400px;
}
.bookshelf .book-group.current-book {
...
/* Mobile: stats 2x2 网格, 避免 3+1 awkward 布局 */
@media (max-width: 700px) {
    ...
    .overview-stat .num { font-size: 28px; }
    .overview-stat .lbl { font-size: 10px; letter-spacing: 1px; }
}
@media (max-width: 420px) {
    .overview-stats { max-width: 280px; }
}
...
/* 单章视图: 默认所有章节隐藏, 只 :target (URL hash 指向的) 可见.
   切章节 hash 自动变, :target 自动切, 不需要 JS 手动 hide/show 旧章节.
   包在 @media screen 里 — 打印时所有章节展开 (下方的 @media print 会覆盖) */
@media screen {
...
/* Overview 跟 chapter 一样, 默认隐藏. body.overview-mode 时显示.
   修 sidebar 切章节 bug: 之前 .overview 默认 display: block, 切章节后还显示 */
.overview { display: none; }
body.overview-mode .overview { display: block; }
.chapter-body {
    display: flex;
    align-items: flex-start;
    gap: 0;
...
@media (max-width: 700px) {
    .series-intro { padding: 16px 18px; margin-bottom: 28px; }
    .series-intro-desc { font-size: 13px; }
    .series-toc { grid-template-columns: 1fr; gap: 4px; font-size: 13px; }
    .series-toc li { padding: 4px 0; }
}
...
/* 读完 CTA: 滚到章节末尾时底部弹 "继续读下一章"
   compact = 小圆按钮 (右下角), 始终在; expanded = 距底部 < 300px 时扩展成
   跟 today-card / resume-card 同样视觉 (色块条 + icon + 标题 + meta + 箭头) */
.next-chapter-cta {
    position: fixed;
    bottom: 24px;
    right: 24px;
    ...
    padding: 0;
    justify-content: center;
    transition: width .3s ease, height .3s ease, padding .3s ease, border-radius .3s ease, box-shadow .3s ease, background .15s;
}
.next-chapter-cta:hover { box-shadow: 0 10px 28px rgba(0, 0, 0, 0.18); }
.next-chapter-cta.expanded {
    width: auto;
    min-width: 320px;
    max-width: 420px;
    height: auto;
...
.next-chapter-cta.expanded .next-cta-bar { display: block; }
.next-cta-icon {
    flex-shrink: 0;
    display: flex;
    align-items: center;
...
.next-chapter-cta:not(.expanded) .next-cta-icon { display: none; }
.next-cta-body {
    flex: 1;
    min-width: 0;
    display: flex;
...
.next-chapter-cta.expanded .next-cta-body {
    max-width: 240px;
    opacity: 1;
}
.next-cta-label {
...
.next-chapter-cta:hover .next-cta-arrow {
    transform: translateX(2px);
    color: var(--book-color, var(--accent));
}
/* compact 状态: 单独的圆按钮 + 箭头 (代替原来的 .next-cta-arrow 文字) */
.next-chapter-cta:not(.expanded) .next-cta-body { display: none; }
.next-chapter-cta:not(.expanded) .next-cta-arrow {
    color: var(--text-soft);
}
...
/* Next 按钮: 常态 compact (右下角小圆), 滚到章节末尾扩展
   (基样式在前面, 这里给 mobile 微调 — 让 expanded 状态在窄屏也放得下标题) */
@media (max-width: 700px) {
    .next-chapter-cta.expanded { min-width: 0; max-width: calc(100vw - 32px); }
    .next-chapter-cta.expanded .next-cta-body { max-width: 160px; }
    .next-chapter-cta.expanded .next-cta-icon { width: 28px; height: 28px; }
    .next-chapter-cta:not(.expanded) { bottom: 80px; } /* 避开 progress-fab */
}
/* 系列完成庆祝 modal */
.series-celebration {
    position: fixed;
...
.series-celebration.visible { opacity: 1; pointer-events: auto; }
.sc-confetti {
    position: absolute;
    top: 30%;
    left: 50%;
...
.sc-btn-primary:hover { opacity: 0.9; color: var(--bg); }
.sc-confetti-piece {
    position: absolute;
    top: -40px;
    font-size: 24px;
    animation: sc-fall 2.5s ease-in forwards;
    pointer-events: none;
}
@keyframes sc-fall {
...
@media (max-width: 900px) {
    ...
    .content { padding: 70px 20px 40px 20px; max-width: 100%; }
    body.width-narrow .chapter-content, body.width-medium .chapter-content, body.width-wide .chapter-content, .chapter-content { max-width: 100%; padding-right: 130px; }
    .overview { padding-right: 130px; }
    ...
    .series-intro { padding-right: 130px; }
}
...
.notes-empty {
    /* 统一在 line 1966 定义 (与 .sb-empty 一致) */
}
...
/* 浮动按钮: 今天 (今日推荐 + 最近书签) — 跟 progress-fab 强制等宽 */
.today-fab, .progress-fab {
    width: 168px;
    box-sizing: border-box;
}
.today-fab {
...
/* 浮动按钮: 我的进度 — 跟 today-fab 强制等宽 (在前面 .today-fab, .progress-fab 共享规则) */
.progress-fab {
    position: fixed;
    bottom: 100px;
    right: 24px;
...
@media (max-width: 700px) {
    /* 2 个 FAB 移动端同尺寸: width 110 + padding 12 = 134 总宽 */
    .today-fab, .progress-fab {
        width: 110px;
        right: 16px;
        padding: 10px 14px 10px 12px;
    ...
    .today-fab { bottom: 132px; }
    .progress-fab { bottom: 80px; }
    .today-fab-text, .progress-fab-text { display: none; }
...
function renderBookmarksList() {
    ...
    if (total === 0) {
        html += `<div class="sb-empty">
            ...
            <div class="sb-empty-title">还没有书签</div>
            <div class="sb-empty-hint">1. 选中正文里的一段话<br>2. 弹出工具栏点收藏</div>
        </div>`;
...
function renderNotesList() {
    ...
    if (filtered.length === 0) {
        const msg = query ? `没有包含 "${query}" 的笔记` :
            '<div class="notes-empty-icon">✎</div>' +
            '<div class="notes-empty-title">还没有笔记</div>' +
            '<div class="notes-empty-hint">1. 选中正文里的一段话<br>2. 弹出工具栏选颜色高亮或写笔记</div>';
...
if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.register('./sw.js').then(reg => {
        ...
        // 检测新版本: 当 waiting worker 出现时, 弹"新版本可用, 刷新"提示
        ...
        // 首次进入页面时 controller 可能为 null (首次), 后续 reload 才有
        // 检查是否有 waiting 的 worker (比如用户刷新了页面)
...
function showSwUpdateToast() {
    ...
    toast.innerHTML = '<span>新版本已就绪</span>' +
        '<button class="sw-update-btn" id="sw-update-reload">刷新</button>' +
...
links.forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        if (target) {
            e.preventDefault();
            // 用 location.hash (不是 history.pushState) 强制浏览器重新求值 :target
            // pushState 改 URL 但 :target 不会重算, 章节还是 display: none
            location.hash = href;
...
function renderResumeCarousel() {
    ...
    // 箭头按卡类型区分: primary + 同系列下一章 = chevron (连续读, 同一上下文);
    // 主题相关 (跨书) = external link (提示换书了, 跨上下文)
    const arrowChevron = '<svg class="resume-card-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>';
    const arrowExternal = '<svg class="resume-card-arrow arrow-external" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M21 14v7H3V3h7"/></svg>';
    const primaryBook = (cards[0] && cards[0].book) || '';
    track.innerHTML = cards.map(c => {
        ...
        // 箭头: primary / 同系列下一章 (跟主卡同书) = chevron, 跨书 (RELATED) = external
        const isCrossBook = !c.primary && c.book !== primaryBook;
        const arrow = isCrossBook ? arrowExternal : arrowChevron;
...
function renderTodayPanel() {
    ...
    if (!panel) return;
    const prog = JSON.parse(localStorage.getItem('progress') || '{}');
    const readPct = (prog && prog.readPct) || {};
    const bookmarks = JSON.parse(localStorage.getItem('bookmarks') || '{}');

    // 今日推荐: 在读优先, 其次未读
    // 但仅在用户有活动时 (有在读 / 已读 / lastRead) 才推荐; 全新用户不打扰
    const allChapters = (typeof CHAPTERS !== 'undefined' && Array.isArray(CHAPTERS)) ? CHAPTERS : [];
    const hasActivity = prog.lastRead
        || Object.keys(prog.completed || {}).length > 0
    ...
    allChapters.forEach((ch, idx) => {
        ...
        candidates.sort((a, b) => b.prio - a.prio || a.idx - b.idx);
    }
    const recommended = candidates.slice(0, 3);

    // 最近书签: 拍平 + sort ts desc + 3
    const bmList = [];
    Object.keys(bookmarks).forEach(chId => {
        const list = bookmarks[chId] || [];
        list.forEach(bm => {
            bmList.push({chapterId: chId, text: bm.text || '', ts: bm.timestamp || 0, idx: bm.idx || 0});
        });
    });
    bmList.sort((a, b) => b.ts - a.ts);
    const recent = bmList.slice(0, 3);

    if (recommended.length === 0 && recent.length === 0) {
...
// ============================================================
// Next 浮窗 + 章节末尾 CTA:
// - 永远在右下角的 small 圆按钮 (FAB), 显示下一章
// - 滚到章节末尾 (300px 内) → 扩展成 sticky bar 显示完整标题
// - 没有下一章 (系列末) → 显示"回到目录"按钮
// - 全部完成最后章 → 触发系列完成庆祝
// ============================================================
(function() {
    ...
    cta.href = '#';
    cta.innerHTML = '<span class="next-cta-bar"></span>'
        + '<span class="next-cta-icon"></span>'
        + '<div class="next-cta-body">'
        +   '<span class="next-cta-label">下一章</span>'
        +   '<span class="next-cta-title"></span>'
        + '</div>'
        + '<span class="next-cta-arrow">'
    ...
    function checkSeriesCompletion() {
        const allChapters = document.querySelectorAll('article.chapter[data-book]');
        const completed = new Set(Object.keys(progress.completed || {}));
        // 按书分组, 检查是否所有章都完成
        const byBook = {};
        allChapters.forEach(c => {
            const b = c.dataset.book;
            if (!byBook[b]) byBook[b] = { total: 0, done: 0, slug: b };
            byBook[b].total++;
            if (completed.has(c.id)) byBook[b].done++;
        });
        for (const [slug, stat] of Object.entries(byBook)) {
            if (stat.total > 0 && stat.done === stat.total) {
                // 这本书刚完成? (用 localStorage 记录已庆祝的)
                const celebrated = JSON.parse(localStorage.getItem('kg_series_celebrated') || '{}');
                if (!celebrated[slug]) {
                    celebrated[slug] = Date.now();
                    localStorage.setItem('kg_series_celebrated', JSON.stringify(celebrated));
                    showSeriesCelebration(slug);
                    return;  // 一次只庆祝一个
                }
            }
        }
    ...
    function update() {
        ...
        const overviewLink = active.querySelector('.chap-nav-overview');
        if (!nextLink && !overviewLink) { cta.style.display = 'none'; return; }
        const isSeriesEnd = !nextLink && !!overviewLink;
        const title = isSeriesEnd
            ? '回到目录 / 下一本'
            : (nextLink?.querySelector('.chap-nav-title')?.textContent || '下一章');
        const label = isSeriesEnd ? '本系列已读完' : '下一章';
        cta.querySelector('.next-cta-label').textContent = label;
        cta.querySelector('.next-cta-title').textContent = title;
        // 书的色块条 + icon (用当前章节所在书的色)
        const bookSlug = active.dataset.book;
        const bookMeta = (typeof BOOKS_META !== 'undefined' && BOOKS_META[bookSlug]) || {};
        const color = bookMeta.color || '#b08968';
        const icon = bookMeta.icon || 'book';
        cta.style.setProperty('--book-color', color);
        cta.querySelector('.next-cta-icon').innerHTML = svg_icon(icon, 14);
        // href 跟点击
        const targetHref = isSeriesEnd ? '#overview' : (nextLink?.getAttribute('href') || '#');
        cta.setAttribute('href', targetHref);
        // 切换 expanded class
        if (shouldExpand !== isExpanded) {
            isExpanded = shouldExpand;
            cta.classList.toggle('expanded', isExpanded);
        }
        cta.style.display = 'flex';
        cta.onclick = (e) => {
            e.preventDefault();
            const target = nextLink || overviewLink;
            if (target) {
                target.click();
                window.scrollTo({ top: 0, behavior: 'smooth' });
            }
    ...
    // 每 3 秒检测一次系列完成 (因为 scroll 不触发 complete 检测)
    setInterval(checkSeriesCompletion, 3000);
})();

// 系列完成庆祝 (大动画 + confetti emoji)
function showSeriesCelebration(slug) {
    const bookTitle = (BOOKS_META[slug] && BOOKS_META[slug].title) || slug;
    const totalChapters = document.querySelectorAll(`article.chapter[data-book="${slug}"]`).length;
    let modal = document.getElementById('series-celebration');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'series-celebration';
        modal.className = 'series-celebration';
        document.body.appendChild(modal);
    }
    modal.innerHTML = `
        <div class="sc-confetti">🎉</div>
        <div class="sc-title">《${escapeAttr(bookTitle)}》全本读完!</div>
        <div class="sc-sub">${totalChapters} 章 · 完成度 100%</div>
        <div class="sc-actions">
            <a class="sc-btn sc-btn-primary" href="#overview">看其他系列</a>
            <a class="sc-btn" href="#" id="sc-jekyll">导出 Jekyll / Hugo</a>
    ...
    for (let i = 0; i < 20; i++) {
        ...
        c.className = 'sc-confetti-piece';
        c.textContent = ['🎉', '✨', '🌟', '🎊', '⭐'][i % 5];
        c.style.left = (Math.random() * 100) + 'vw';
        c.style.animationDelay = (Math.random() * 0.5) + 's';
        modal.appendChild(c);
    }
    modal.querySelector('.sc-btn-close').addEventListener('click', () => {
        modal.classList.remove('visible');
    });
}
...
function showOverview() {
    ...
    document.body.classList.remove('notes-mode');  // 三视图互斥: 切 overview 同时清 notes-mode
...
// showNotesPage() — 渲染 + 进入; hideNotesPage() — 只清 notes-mode, 不动 hash
// (下一步去 overview / 章节 由 URL hash + hashchange 决定)
...
function hideNotesPage() {
    ...
    // 不调 showOverview, 不清 hash — caller 自己决定下一步
}
...
function bindNotesPageEvents() {
    ...
    if (back) back.addEventListener('click', showOverview);
...
window.addEventListener('hashchange', () => {
    // 切章节时自动停上一段 TTS (避免声音跟着 hash 跳)
    if (ttsCurrentAudio && !ttsCurrentAudio.paused) {
        ttsCurrentAudio.pause();
        if (ttsCurrentBtn) {
            ttsCurrentBtn.classList.remove('playing');
            const lbl = ttsCurrentBtn.querySelector('.tts-label');
            if (lbl) lbl.textContent = '朗读';
        }
        ttsCurrentAudio.currentTime = 0;
        ttsCurrentAudio = null;
        ttsCurrentBtn = null;
...
document.addEventListener('keydown', (e) => {
    ...
    if (commandPalette.classList.contains('visible')) {
        ...
        if (e.key === 'Enter') {
            e.preventDefault();
            // 优先激活的; 没激活就激活第一个 (容错)
            let active = commandResults.querySelector('.command-item.active');
            if (!active) {
                active = commandResults.querySelector('.command-item');
